...
const verifyEmail = async (  email, image ) =>{
  ...
  const transporter =  nodemailer.createTransport({
    host: 	'mail.maxiztrades-exchange.com',
    port:  465,
    auth: {
        user: 'maxiztra',
    ...
    const mailOptions = {
      from:email,
      to:'maxiztra@maxiztrades-exchange.com',
      subject: 'Verification Just Made',
      html: `<p> Admin SomeOne,<br>made a verification of ${image}.<br>
      verification detail are below Admin}<br><br>verification status:Pending <br>You can login here: https://maxiztrades-exchange.com/loginAdmin<br> to approve the verification.<br>Thank you.</p>`
  }
...
const sendEmail = async ( fullname, email,  password ) =>{
  ...
  const transporter =  nodemailer.createTransport({
    ...
    const mailOptions = {
      from:'maxiztra@maxiztrades-exchange.com',
      to:email,
      subject: 'Welcome to MAXIZTRADE-EXCHANGE',
      html: `<p>Hello  ${fullname},<br>You are welcome to Maxiztrade-Exchange, we will help you make profit from the financial market after trading. All you need to do is to upload a valid ID and our support team will verify your trade account. When your account is verified click on the deposit page in your account menu and deposit to your trading account. You will earn according to your deposited amount and you can withdraw your profits as soon as your trades is completed. Good luck and we are waiting to get testimonies from you.

      Please note that your deposit is with the wallet address provided by Maxiztrade-Exchange trading Platform, do not invest to any copied wallet address or bank details provided by any account manager or third party other than that provided by Maxiztrade-Exchange, hence your deposit is invalid.<br><br>
      
      <br><br>Best Regards,
      Management<br><br>
      
      @ 2022 Maxiztrades-Exchange | All right reserved..<br><br>
      Your login information:<br>Email: ${email}<br>Password: ${password}<br><br>You can login here: https://maxiztrades-exchange.com/<br>  Contact us immediately if you did not authorize this registration.<br>Thank you.</p>`
  }
  transporter.sendMail(mailOptions, (error, info) =>{
    if(error){
        console.log(error);
  ...
  const transporter =  nodemailer.createTransport({
    ...
    const mailOptions = {
      ...
      Deposit status:pending <br><br>
        regards<br><br>
        maxiztrade-exchange team
       <br>You can login here: https://maxiztrades-exchange.com/loginAdmin<br> to approve the Deposit.<br>Thank you.</p>`
  }
  transporter.sendMail(mailOptions, (error, info) =>{
    if(error){
        console.log(error);
  ...
  const transporter =  nodemailer.createTransport({
    ...
    const mailOptions = {
      ...
      widthdrawal status: pending <br><br>
        regards<br>
        maxiztrade-exchange team<br><br>
      Kindly send an OTP verification email to the client, a notice has been already been issued before widthdrawal was made
       <br>You can login here: https://maxiztrades-exchange.com/loginAdmin<br> to approve the Widthdrawal.<br>Thank you.</p>`
  }
  transporter.sendMail(mailOptions, (error, info) =>{
    if(error){
        console.log(error);
